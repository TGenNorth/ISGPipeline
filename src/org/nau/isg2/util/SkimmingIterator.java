...
public class SkimmingIterator<T> implements Iterator<Map<String, T>> {
    ...
    final Map<String, Iterator<T>> iterMap = new HashMap<String, Iterator<T>>();
    final Map<String, T> currentElements = new HashMap<String, T>();
    ...
    public Map<String, T> next() {
        ...
        for (final String key : currentElements.keySet()) {
    ...
    private void advanceAll() {
        for (final String key : iterMap.keySet()) {
            advance(key);
        }
    }

    private void advance(Collection<String> keys) {
